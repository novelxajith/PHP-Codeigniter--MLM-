    ...
    <div class="col-12">
      <div class="card">
          ...
          <div class="col-xl-12">
            <div class="nav-align-top mb-4">
              <!--<ul class="nav nav-pills mb-3 nav-fill" role="tablist">-->
                ...
                ?>
            <?php
             $usertypebadge_main = $this->db->select_max('credit')->where('username',  $this->session->userdata('ablisusername'))->where('remark', 'Activation')->get('pin')->row()->credit + 0;
        ...
        <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                ...
                <li class="nav-item">
                  <a id="<?=$badgetype_main?>" href="<?=BASEURL?>admin/team_management/<?=$this->session->userdata('ablisusername')?>/wing1" class="nav-link  btn btn-md <?=$one?>"> Wing 1
                
                </a>
                
              </li>
              <li class="nav-item">
                <a id="<?=$badgetype_left?>" class="nav-link btn  btn-md <?=$two?>" href="<?=BASEURL?>admin/team_management/<?=$left?>/wing2">Wing 2
                 
              </a>

            </li>
            <li class="nav-item">
              <a id="<?=$badgetype_right?>" class="nav-link btn btn-md <?=$three?>" href="<?=BASEURL?>admin/team_management/<?=$right?>/wing3">Wing 3
        ...
        <div class="tab-content">
                        <div class="tab-pane fade show active" id="navs-pills-justified-home" role="tabpanel">
                           <div class="row mt-2 d-flex justify-content-center">
                              ...
                              <div class="col-lg-5 mt-3">
                                 ...
                                 <a class="btn btn-primary m-1 waves-effect waves-light" href="<?=BASEURL?>admin/registration/<?=$this->session->userdata('ablisusername')?>" id="myLink" 
                                 <span class="tf-icons mdi mdi-plus me-1"></span> Add user
                                 </a>
                              </div>
                              <?php $left_free=$this->db->select('username')->where('user_type','sub')->where('ref_id',$this->session->userdata('ablisusername'))->where('position','left')->get('user_role')->row()->username; 
                                 $right_free=$this->db->select('username')->where('user_type','sub')->where('ref_id',$this->session->userdata('ablisusername'))->where('position','right')->get('user_role')->row()->username;
                                 ?>
                           </div>
                                          ...
                                          <div class="card border <?=$badge_left?> user">
                                             <div class="card-body ">
                                                <div class="row mobile">
                                                   ...
                                                   <div class="col">
                                                           <a href="<?=BASEURL?>admin/team_management/<?=$key['child_id']?>/<?=$wing?>">
            ...
            <?php }if($usertype_checkkk['user_type'] == "sub" && $usertype_checkkk['position'] == "left" && $usertype_checkkk['ref_id'] != $this->session->userdata('ablisusername')){ ?>
                                                      <h4> <span class="mobileh5 username">Wing 2</span></h4>
                                                      <h5><span class="mobileh6 position" ><?php echo $name['first_name'] ?> <?php echo $name['last_name'] ?></span></h5>
            <?php } if($usertype_checkkk['user_type'] == "sub" && $usertype_checkkk['position'] == "right" && $usertype_checkkk['ref_id'] != $this->session->userdata('ablisusername')) { ?>
                                                      <h4> <span class="mobileh5 username">Wing 3</span></h4>
                                                      <h5><span class="mobileh6 position" ><?php echo $name['first_name'] ?> <?php echo $name['last_name'] ?></span></h5>
            <?php } if($usertype_checkkk['user_type'] == "sub" && $usertype_checkkk['ref_id'] == $this->session->userdata('ablisusername')) { ?>
                                          ...
                                          <div class="card border <?=$badge_right?> user">
                                             <div class="card-body ">
                                                <div class="row mobile">
                                                   ...
                                                   <div class="col">
                                                       <a href="<?=BASEURL?>admin/team_management/<?=$keyy['child_id']?>/<?=$wing?>">
            ...
            <?php }if($usertype_checkk['user_type'] == "sub" && $usertype_checkk['position'] == "left" && $usertype_checkk['ref_id'] != $this->session->userdata('ablisusername')){ ?>
                                                      <h4> <span class="mobileh5 username">Wing 2</span></h4>
                                                      <h5><span class="mobileh6 position" ><?php echo $namee['first_name'] ?> <?php echo $namee['last_name'] ?></span></h5>
            <?php } if($usertype_checkk['user_type'] == "sub" && $usertype_checkk['position'] == "right" && $usertype_checkk['ref_id'] != $this->session->userdata('ablisusername')) { ?>
                                                      <h4> <span class="mobileh5 username">Wing 3</span></h4>
                                                      <h5><span class="mobileh6 position" ><?php echo $namee['first_name'] ?> <?php echo $namee['last_name'] ?></span></h5>
            <?php } if($usertype_checkk['user_type'] == "sub" && $usertype_checkk['ref_id'] == $this->session->userdata('ablisusername')) { ?>
                                                       <h4> <span class="mobileh5 username">Bonus ID</span></h4>
                                                      <h5><span class="mobileh6 position" ><?php echo $namee['first_name'] ?> <?php echo $namee['last_name'] ?></span></h5>
            <?php } ?>
                                                       </a>
...
<script>
  $(document).ready(function() {
    $('#button1').click(function() {
      $('#myLink').attr('href', '<?=BASEURL?>admin/registration/<?=$this->session->userdata('ablisusername')?>');
    });

    $('#button2').click(function() {
      $('#myLink').attr('href', '<?=BASEURL?>admin/registration/<?=$left_free?>');
    });

    $('#button3').click(function() {
      $('#myLink').attr('href', '<?=BASEURL?>admin/registration/<?=$right_free?>');
    });
  });
</script>
<script>
...
foreach($left_team as $left_team_key){
    ...
    $total_left_plus += $this->db->select('credit')->where('username',$left_team_key['child_id'])->where('type','Plus')->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_plus_left_bv= $total_left_plus/1100 ;
    $total_left_prime += $this->db->select('credit')->where('username',$left_team_key['child_id'])->where('type','Prime')->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_prime_left_bv=$total_left_prime/10000;
    
    $total_left_plus_today += $this->db->select('credit')->where('username',$left_team_key['child_id'])->where('date(entry_date)',date('Y-m-d'))->where('type','Plus')->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_plus_left_bv_today= $total_left_plus_today/1100 ;
    $total_left_prime_today += $this->db->select('credit')->where('username',$left_team_key['child_id'])->where('date(entry_date)',date('Y-m-d'))->where('type','Prime')->where('remark','Activation')->get('pin')->row()->credit+0 ;
...
foreach($right_team as $right_team_key){
    ...
    $total_right_plus += $this->db->select('credit')->where('username',$right_team_key['child_id'])->where('type','Plus')->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_plus_right_bv= $total_right_plus/1100 ;
    $total_right_prime += $this->db->select('credit')->where('username',$right_team_key['child_id'])->where('type','Prime')->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_prime_right_bv=$total_right_prime/10000;
    
    $total_right_plus_today += $this->db->select('credit')->where('username',$right_team_key['child_id'])->where('type','Plus')->where('date(entry_date)',date('Y-m-d'))->where('remark','Activation')->get('pin')->row()->credit+0 ;
    $total_plus_right_bv_today= $total_right_plus_today/1100 ;
    $total_right_prime_today += $this->db->select('credit')->where('username',$right_team_key['child_id'])->where('type','Prime')->where('date(entry_date)',date('Y-m-d'))->where('remark','Activation')->get('pin')->row()->credit+0 ;
