      ...
      <div class="card">
        ...
        <div class="tab-content">
            ...
            <?php } ?>
                        ...
                        $lfgg=$this->db->like('team',$key['child_id'])->get('tree')->result_array();
                        $today_left_plus_wing1=0;
                        $today_left_prime_wing1=0;
                        foreach($lfgg as $jkj){
                            $today_left_plus_wing1 += $this->db->select_sum('credit')->where('remark','Activation')->where('username',$jkj['child_id'])->where('type','Plus')->where('date(entry_date) ',date('Y-m-d'))->get('pin')->row()->credit+0;
                            $today_left_prime_wing1 += $this->db->select_sum('credit')->where('remark','Activation')->where('username',$jkj['child_id'])->where('type','Prime')->where('date(entry_date) ',date('Y-m-d'))->get('pin')->row()->credit+0;
                        }
                        
                
                
                $today_left_plus_wing1_count=($today_left_plus_wing1/1100);
                $today_left_prime_wing1_count=($today_left_prime_wing1/10000);
                
                $today_left_plus_wing1_total=$today_left_plus_wing1_count*300;
                $today_left_prime_wing1_total=$today_left_prime_wing1_count*1000;
            ...
            <?php } ?>
                ...
                $lfggyff=$this->db->like('team',$keyy['child_id'])->get('tree')->result_array();
                    $today_right_plus_wing1=0;
                    $today_right_prime_wing1=0;
                    foreach($lfggyff as $jkjoii){
                        $today_right_plus_wing1 = $this->db->select_sum('credit')->where('remark','Activation')->where('username',$jkjoii['child_id'])->where('type','Plus')->where('date(entry_date) ',date('Y-m-d'))->get('pin')->row()->credit+0;
                        $today_right_prime_wing1 = $this->db->select_sum('credit')->where('remark','Activation')->where('username',$jkjoii['child_id'])->where('type','Prime')->where('date(entry_date) ',date('Y-m-d'))->get('pin')->row()->credit+0;    
                            
                        }
                        
                
                
                $today_right_plus_wing1_count=($today_right_plus_wing1/1100);
                $today_right_prime_wing1_count=($today_right_prime_wing1/10000);
                
                $today_right_plus_wing1_total=$today_right_plus_wing1_count*300;
                $today_right_prime_wing1_total=$today_right_prime_wing1_count*1000;
