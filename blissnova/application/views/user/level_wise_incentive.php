   ...
   <div class="row">
      <div class="col-12">
         ...
         <?php 
               $count=1;
               if(!empty($level)){
               $inc=$this->db->where('level',$level)->where('username',$this->session->userdata('ublisusername'))->get('account_sub')->result_array();
               }
               foreach($inc as $key=>$income){ 
               ?>
                     <tr>
                        <td><?=$count++?></td>
                        ...
                        <?php $name=$this->db->where('username',$income['description'])->get('user_role')->row_array(); ?>
