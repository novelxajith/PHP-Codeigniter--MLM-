            ...
            <div class="nav-align-top mb-4 mt-3 ">
               ...
               <ul class="nav nav-pills  mb-3" >
                  <li class="nav-item" role="presentation">
                     <a href="<?=BASEURL?>user/left_team/<?=$this->session->userdata('ublisusername')?>/wing1" class="nav-link waves-effect waves-light <?=$one?>" >
                     Wing 1
                     </a>
                  </li>
                  <?php $left=$this->db->where('user_type','sub') ->where('ref_id',$this->session->userdata('ublisusername'))->where('position','left')->get('user_role')->row_array(); 
                     ...
                     ?>         
                  <li class="nav-item">
                     <a href="<?=BASEURL?>user/left_team/<?=$left['username']?>/wing2" class="nav-link waves-effect waves-light <?=$two?>" >
                     Wing 2
                     </a>
                  </li>
                  <li class="nav-item">
                     <a href="<?=BASEURL?>user/left_team/<?=$right['username']?>/wing3" class="nav-link waves-effect waves-light <?=$three?>" >
                     Wing 3
                     </a>
                  </li>
               </ul>
               <div class="tab-content">
                  <div class="tab-pane fade active show" id="navs-pills-top-home" role="tabpanel">
                     <h5 class="card-header">A Team</h5>
                     <div class="card-datatable table-responsive pt-0">
                        <table id="example" class="datatables-basic table table-bordered" style="width:100%">
                           ...
                           <tbody>
                              <?php $left_user=$this->db->where('position','left')->where('parent_id',$username)->get('tree')->row_array();
                                 if(!empty($left_user)){
                                 ...
                                 ?>
                              <?php $count=1; foreach($left_team as $key){ ?>
                              <tr>
                                 <td><?=$count++?></td>
                                 <td><?=$key['entry_date']?></td>
                                 <?php    $users=$this->db->where('username',$key['child_id'])->get('user_role')->row_array();?>                                             
                                 ...
                                 <?php    $plan=$this->db->select_max('credit')->where('username',$key['child_id'])->where('remark','Activation')->get('pin')->row()->credit+0;?>     
                                 <td><?=$plan?></td>
                                 <td><?=$users['ref_id']?></td>
